  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 p-5 flex-1 min-h-0">
        ...
        {/* APC — cost and mode as separate badges */}
        {(event.apc_results?.length ?? 0) > 0 && (
          <div className="shrink-0 flex flex-wrap gap-1">
            {event.apc_results!.slice(0, 2).flatMap((a) => [
              <span key={`${a.id}-cost`} className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-violet-50 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300 border border-violet-100 dark:border-violet-800/50">
                {a.apc_currency ?? ''} {a.apc_value ?? '—'}
              </span>,
              ...(a.apc_mode_normalized || a.apc_mode_raw
                ? [
                    <span key={`${a.id}-mode`} className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 border border-amber-100 dark:border-amber-800/50">
                      {a.apc_mode_normalized ?? a.apc_mode_raw}
                    </span>,
                  ]
                : []),
            ])}
            {(event.apc_results!.length > 2) && <span className="text-[10px] text-neutral-400">+{event.apc_results!.length - 2}</span>}
          </div>
        )}
      </div>
